    ...
    1: [function (require, module, exports) {
      ...
      function attach(term, socket, bidirectional, buffered) {
        ...
        function displayData(data) {
          ...
          if (data.type == 'data') {
            _displayData(data.data)
          } else if (data.type == 'resize') {
            if (term._doChanged) {
              term._doChanged({
                code: 22,
                msg: data.msg || 'resize 失败'
              })
            }
            console.error(data.msg || 'resize 失败');
          }
        }

        function _displayData(str, data) {
          if (buffered) {
        ...
        addonTerminal.__resizeTerm = function (size) {
          if (!addonTerminal.__socket) {
            return;
          }
          ...
          data = JSON.stringify(data);
          addonTerminal.socket.send(data);
        }
        ...
        if (bidirectional) {
          ...
          addonTerminal._core.register(addonTerminal.addDisposableListener('paste', addonTerminal.__sendData));
        }
        addonTerminal._core.register(addSocketListener(socket, 'resize', addonTerminal.__resizeTerm));
        addonTerminal._core.register(addSocketListener(socket, 'close', function () {
          return detach(addonTerminal, socket);
        }));
        addonTerminal._core.register(addSocketListener(socket, 'error', function () {
          return detach(addonTerminal, socket);
        }));
      }
      ...
      function detach(term, socket) {
        ...
        addonTerminal.off('resize', addonTerminal.__resizeTerm);
        addonTerminal.off('paste', addonTerminal.__sendData);
